 ...
 *
 * Conversation history is owned by the client and passed in on each request —
 * no shared mutable state, no cross-user leakage, no memory growth.
 */
@Service
public class ChatbotService {
    ...
    public String chat(String userMessage, List<ChatRequest.ChatMessageDto> history) {
        try {
            StringBuilder conversation = new StringBuilder();
            conversation.append(buildSystemPrompt()).append("\n\n");

            for (ChatRequest.ChatMessageDto msg : history) {
            ...
            if (response == null || response.isEmpty()) {
                log.warn("Empty response from LMM");
            ...
            log.error("Error calling LMM: {}", e.getMessage(), e);
